...
if ($_GET[saveTitleIcon]) {
	...
	if (strpos($_GET[path],"-")) {
		if (!is_array($aUsers)) {
			echo $q = "DELETE FROM conf_relation_visibility WHERE path = '$_GET[path]'";
			q($q);
			exit();
		} else {
			$q = "SELECT id FROM conf_relation_visibility WHERE path = '$_GET[path]'";
			$a = q($q);
			if (count($a)) {
				echo $q = "UPDATE conf_relation_visibility SET users = '".serialize($aUsers)."' WHERE path = '$_GET[path]'";
			} else {
				echo $q = "INSERT INTO conf_relation_visibility SET users = '".serialize($aUsers)."', path = '$_GET[path]'";
			}
		}
		q($q);
	} else {
		echo $q = "UPDATE conf_tables SET users = '".serialize($aUsers)."' WHERE id='$_GET[showPossibleRelations]'";
		q($q);
        echo $q = "UPDATE conf_relation_visibility SET users = '".serialize($aUsers)."' WHERE path = '".$_GET[showPossibleRelations]."-'";
        q($q);
		if (!is_array($aUsers)) {
			exit();
		}
	...
	$q = "SELECT ".$a[0][ntomAjaxDisplayTitleField]." title FROM ".$tt[name]." WHERE ".$a[0][ntomAjaxDisplayTitleField]." LIKE '$_GET[value]%'";
	$a = dbQuery($q);
	$r = array();
	foreach ($a as $k => $v) 
